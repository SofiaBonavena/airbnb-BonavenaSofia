...
// USE STATE: SIRVE PARA GUARADR VARIABLES

const App = () => {

    
  const [hoteles, setHoteles] = useState([]);
  const [hotelesFiltered, setHotelesFiltered] = useState([]); // Para que muestr todos los hoteles filtrados
  ...
  const handleFilter = ({ dateFrom, dateTo, country, rooms, price }) => {
    return hoteles.filter(
      (hotel) =>
       ...
       hotel.country === (country !== 'select' ? country : hotel.country) && // Country es distinto de select? Sino poneme hotel.country
       ...
       // Vamos a entregarle los filtros de select
       );
  };
